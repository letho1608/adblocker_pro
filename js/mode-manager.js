...
export function setDefaultFilteringMode(afterLevel) {
    // Tự động nâng cấp lên chế độ tối ưu nếu user chọn chế độ thấp hơn
    if (afterLevel < 3) {
        afterLevel = 3; // Luôn sử dụng chế độ Complete để có hiệu suất tối đa
    }
...
export async function syncWithBrowserPermissions() {
    ...
    if ( beforeMode > MODE_BASIC && hasBroadHostPermissions === false ) {
        // Vẫn cố gắng sử dụng chế độ tối ưu ngay cả khi không có quyền rộng
        ...
        await setDefaultFilteringMode(3); // Level 3 = Complete
        modified = true;
    }
